...
// 将 tarball 保存到磁盘，将元数据写入数据库
func (s *Service) Publish(body *PublishBody, uploaderID uint, uploaderName string) error {
	// 1. 创建或更新包记录
	pkg, err := s.upsertPackage(body.Name, body.Description, body.Readme, body.DistTags)
	if err != nil {
	...
	for version, metaRaw := range body.Versions {
		...
		err := s.db.Where("package_id = ? AND version = ?", pkg.ID, version).First(&existing).Error
		if err == nil {
		...
		if err := s.db.Create(&npmVer).Error; err != nil {
